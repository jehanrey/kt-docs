...
Firstly, we can declare our `entryPoint`, `callbackUrl`, `logoutUrl`, `issuer`, and `idpCert` inside our `.env` file. In this case, here's what our `.env.development` would look like:
...
DIR employs an authentication system that leverages http-only cookies to maintain user state. We makes use of `middleware.ts` to validate the the cookies of incoming requests. When an incoming request do not contain the valid tokens, the requests are redirected to `/api/login` to trigger the login flow.
...
The request is redirected to the SSO login page through the SAML url which is generated using the config that we defined [earlier](#implementation).
